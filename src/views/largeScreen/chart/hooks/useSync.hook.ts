import { getUUID } from '/@/utils/largeScreen';
import { useChartEditStore } from '/@/store/modules/largeScreen/chartEditStore/chartEditStore';
import {
  ChartEditStoreEnum,
  ChartEditStorage,
} from '/@/store/modules/largeScreen/chartEditStore/chartEditStore.d';
  ...
  const updateComponent = async (projectData: ChartEditStorage, isSplace = false) => {
    ...
    projectData.componentList.forEach(async (e: CreateComponentType) => {
      if (!window['$vue'].component(e.chartConfig.chartKey)) {
        window['$vue'].component(e.chartConfig.chartKey, fetchChartComponent(e.chartConfig));
        window['$vue'].component(e.chartConfig.conKey, fetchConfigComponent(e.chartConfig));
      }
    });
    // 数据赋值
    for (const key in projectData) {
      ...
      if (key === ChartEditStoreEnum.COMPONENT_LIST) {
        for (const comItem of projectData[key]) {
          // 补充 class 上的方法
          const newComponent: CreateComponentType = await createComponent(comItem.chartConfig);

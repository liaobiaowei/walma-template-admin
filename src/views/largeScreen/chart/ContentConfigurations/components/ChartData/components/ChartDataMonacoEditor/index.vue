<template>
  <template v-if="targetData.filter">
    <n-card>
      ...
      <!-- 函数体 -->
      <div class="go-ml-4">
        <n-code :code="targetData.filter" language="typescript" />
      </div>
      <p>}</p>
      <template #footer>
        <n-space justify="end">
            ...
            <n-space :size="15" vertical>
              <div class="editor-data-show">
                <n-space>
                  ...
                  <n-code :code="toString(sourceData)" language="json" :word-wrap="true" />
                </n-space>
              </div>
              <div class="editor-data-show">
                <n-space>
                  <n-text depth="3">过滤器结果：</n-text>
                  <n-code :code="filterRes" language="json" :word-wrap="true" />
                </n-space>
              </div>
            </n-space>
          </n-scrollbar>
...
<script lang="ts" setup>
  ...
  import { RequestHttpEnum, RequestDataTypeEnum, ResultEnum } from '/@/enums/largeScreen/httpEnum';
  import { icon } from '/@/plugins';
  import { goDialog, toString } from '/@/utils/largeScreen';
